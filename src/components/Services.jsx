...
export default function Services() {
  ...
  return (
    <section
      ...
      className="relative max-w-[1400px] mx-auto py-20 md:py-32 px-4 md:px-8 isolate z-10 overflow-hidden"
    >
      {/* ── Background Glows ── */}
      <div
        aria-hidden="true"
      ...
      <div
        aria-hidden="true"
        className="absolute top-[10%] -left-[10%] w-[400px] h-[400px] md:w-[600px] md:h-[600px] bg-[radial-gradient(circle_at_center,rgba(0,200,180,0.12)_0%,transparent_60%)] blur-[80px] rounded-full pointer-events-none -z-10 animate-[auroraSway_10s_ease-in-out_infinite]"
      />
      <div
        aria-hidden="true"
        className="absolute bottom-[10%] -right-[10%] w-[400px] h-[400px] md:w-[600px] md:h-[600px] bg-[radial-gradient(circle_at_center,rgba(227,180,212,0.12)_0%,transparent_60%)] blur-[80px] rounded-full pointer-events-none -z-10 animate-[auroraSway_12s_ease-in-out_infinite_reverse]"
      />
      ...
      {/* ── Mobile Carousel ── */}
      <div className="md:hidden relative w-full h-[520px] flex items-center justify-center mt-4">
        
        {/* Botón Prev */}
        <button
          onClick={prevSlide}
          aria-label="Servicio anterior"
          className="absolute left-0 z-30 flex items-center justify-center w-12 h-12 rounded-full bg-[#000318]/50 backdrop-blur-md border border-white/10 text-white/80 transition-all active:scale-90 hover:bg-white/10 hover:text-white"
        >
        ...
        <div className="relative w-full h-full flex items-center justify-center pointer-events-none">
          {services.map((s, i) => {
            ...
            return (
              <div
                key={i}
                className="absolute w-[82%] sm:w-[75%] h-full transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)]"
                style={{
                  transform: `translateX(${offset * 108}%) scale(${isActive ? 1 : 0.88})`,
        ...
        <button
          ...
          className="absolute right-0 z-30 flex items-center justify-center w-12 h-12 rounded-full bg-[#000318]/50 backdrop-blur-md border border-white/10 text-white/80 transition-all active:scale-90 hover:bg-white/10 hover:text-white"
        >
          <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function ServiceCard({ title, desc, img, isMobile }) {
  return (
    ...
    >
      ...
      <div className={`relative w-full ${isMobile ? 'h-[220px]' : 'h-[240px] lg:h-[260px]'} overflow-hidden bg-[#000318]/50 border-b border-white/5`}>
        <div className="absolute inset-0 bg-gradient-to-t from-[#000318] via-transparent to-transparent opacity-80 mix-blend-multiply z-10 pointer-events-none" />
        <img
          src={img}
          alt={title}
          loading="lazy"
      ...
      <div className="flex flex-col flex-grow items-center p-6 md:p-8 lg:p-10 relative z-20">
        ...
        <p className="m-0 text-[0.95rem] md:text-base text-[rgba(220,230,245,0.7)] leading-relaxed font-medium">
          {desc}
        </p>
      </div>
    </article>
